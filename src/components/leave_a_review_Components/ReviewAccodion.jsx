...
const faqData = [
  {
    ...
    answer:
      "If you used aircraft at this school, please check the box below",
  },
  {
    id: 2,
...
const ReviewAccodion = () => {
  return (
    <div className=' py-10'>
      ...
      >
        {faqData.map((faq) => (
          <AccordionItem key={faq.id} value={`item-${faq.id}`} className={"my-3 border rounded-md"}>
            <AccordionTrigger className="text-xl font-semibold bg-[#F3F4F6] px-5">
              {faq.question}
            </AccordionTrigger>
            <AccordionContent className="flex flex-col mt-4 px-5 gap-4 text-balance text-base">
              <p>{faq.answer}</p>
            </AccordionContent>
          </AccordionItem>
        ))}
